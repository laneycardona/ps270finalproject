...
post_treatment <- post_treatment |>
  mutate(post_treatment_group = case_when(
    ideology_group %in% c("1. Most Liberal", "2. Liberal") ~ "Liberals",
    ideology_group == "3. Moderate" ~ "Moderates",
    ideology_group %in% c("4. Conservative", "5. Most Conservative") ~ "Conservatives"
  ))
...
Running a regression to determine the relationship between ideology groups and average change in attitudes
```{r}
library(broom)
reg2 <- lm(average_change ~ binary_variable, data = treated)
reg2_table <- tidy(reg2)
...
Creating a visualization for the regression output
```{r}
reg2_plot <- ggplot(reg2_table, aes(x = term, y = estimate)) +
  geom_point(size = 3, color = c("blue", "red")) +
  geom_errorbar(aes(ymin = estimate - std.error, ymax = estimate + std.error), width = 0.2) +
  labs(
    title = "Regression Results treated group only",
    x = "Ideology",
    y = "Coefficient",
...
reg2_plot
```
This graph shows a visualization of the regression output above. Based on the scale of the experiment, the graph indicates that on average, liberals experienced a shift towards more negative attitudes on immigration policies, while conservatives shifted toward more favorable views. However, the somewhat wide error bar for the binary variable suggest that this estimate is moderately less precise, meaning that there is a little more uncertainty regarding the reliability for this estimate. 



## Conclusion 

Initially, the data aligned with my hypothesis. Conservatives started out with the most negative views on immigration, and on average, after encountering the confederates, they experienced a shift towards more favorable attitudes on immigration compared to liberals. While they did shift toward more positive views, their shift in overall attitudes was much smaller than liberals', indicating that they acted more conservatively. Additionally, unlike what I expected, liberals actually experienced a shift toward more negative attitudes. The p-value from the regression output indicates that the main coefficient is statistically significant, supporting these results that show there is a meaningful difference in the attitude shifts between the two ideology groups. Therefore, my hypothesis was not supported by the data. The presence of external influences as confounding factors is one of the most significant limitations of this research. Because this experiment used a before-and-after research design, the results don't consider the possible external influences, such as peer interactions, media exposure, or major events that could affect the participants' shifts in attitudes. Therefore, through the fundamental problem of causal inference, it is impossible to determine the confederate interaction is the direct cause of the attitude shifts between the two ideology groups. To improve my analysis, I would increase the sample size of the experiment. While the current standard error is not extremely large, a bigger sample size would reduce the margin of error that we do have, creating more precise and accurate estimates of the regression coefficients. There were significant disparities in attitude shifts between the specific categories and the overall combined ideology groups. Therefore, a larger sample size would raise the generalizability of the results, making them representative of a wider population. This would improve the reliability and certainty of the analysis, making it more likely that a true effect and difference can be observed if it exists. 
